...
int main() {
  ...
  config.controls.max_iterations = 250;
  config.controls.min_iterations = 10;
  config.controls.projection_dt = 0.15;
  config.output_dir = "build/test_projection_smoke_output";

  cfd::ProjectionSolver solver(config);
  const cfd::SolveSummary summary = solver.run();

  assert(summary.solver == cfd::FlowSolverKind::kProjection);
  ...
  for (int i = 1; i <= solver.grid().nx(); ++i) {
    ...
    expect_near(
        fields.u(i, solver.grid().ny() + 1),
        2.0 * config.lid_velocity - fields.u(i, solver.grid().ny()));
